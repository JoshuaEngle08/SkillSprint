...
document.addEventListener('DOMContentLoaded', function(){
  ...
  (function initChat(){
    ...
    function setTyping(on){
      const existing = chatMessagesEl.querySelector('.msg.typing');
      if(on){
        if(!existing){
          const t = document.createElement('div'); t.className = 'msg bot typing'; t.textContent = 'Typing...'; chatMessagesEl.appendChild(t); chatMessagesEl.scrollTop = chatMessagesEl.scrollHeight;
        }
    ...
    // Open/close
    function openChat(){ chatWidget.setAttribute('aria-hidden','false'); chatPanel.classList.add('open'); chatInput.focus(); }
    function closeChat(){ chatWidget.setAttribute('aria-hidden','true'); chatPanel.classList.remove('open'); chatToggle.focus(); }

    chatToggle && chatToggle.addEventListener('click', ()=>{ const open = chatWidget.getAttribute('aria-hidden') === 'false'; open ? closeChat() : openChat(); });
